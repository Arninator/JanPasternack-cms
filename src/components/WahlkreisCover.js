...
class WahlkreisCoverTemplate extends React.Component {
...
export default function WahlkreisCover() {

  return (
    <StaticQuery
      query={graphql`
        query WahlkreisCoverQuery {
          allMarkdownRemark(
            sort: { order: DESC, fields: [frontmatter___date] }
            filter: { frontmatter: { templateKey: { eq: "wahlkreis-cover" } } }
          ) {
            edges {
              node {
                id
      ...
      `}
      render={(data, count) => < WahlkreisCoverTemplate data={data} count={count} />}
    />
  );
}
