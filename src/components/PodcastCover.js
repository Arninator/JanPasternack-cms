...
class PodcastCoverTemplate extends React.Component {
...
export default function PodcastCover() {
  ...
  return (
    <StaticQuery
      query={graphql`
        query PodcastCoverQuery {
          allMarkdownRemark(
            ...
            filter: { frontmatter: { templateKey: { eq: "podcast-cover" } } }
          ) {
            edges {
              node {
                id
      ...
      `}
      render={(data, count) => < PodcastCoverTemplate data={data} count={count} />}
    />
  );
}
